...
type RelayConfig struct {
	...
	PrivateID security.PrivateID `json:"privateId" yaml:"privateId"`
}

type relayRequest struct {
	PublicIDHash []byte `json:"publicIDHash" yaml:"publicIDHash"`
	Timestamp    string `json:"timestamp" yaml:"timestamp"`
	Signature    []byte `json:"signature" yaml:"signature"`
}

// OpenRelay creates a new Relay store.that forwards all operations to an inner storage
// A relay is a web service that returns a store.endpoint given a public ID. The endpoint
...
func OpenRelay(id string, c RelayConfig) (Store, error) {
	...
	// The response contains the store.endpoint configuration encrypted with the private ID
