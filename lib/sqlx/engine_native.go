...
func OpenEngine(driverName, dataSourceName string) (*Engine, error) {
	...
	if dataSourceName != MemoryDB && driverName == "sqlite3" {
		...
		if errors.Is(err, os.ErrNotExist) {
			...
			if err != nil {
				return nil, core.Error(core.DbError, "Cannot create SQLite db file %s", dataSourceName, err)
			}
		} else if err != nil {
			return nil, core.Error(core.DbError, "Cannot access SQLite db file %s", dataSourceName, err)
		}
	}
	...
	if err != nil {
		return nil, core.Error(core.DbError, "Cannot create directory %s for SQLite db %s", dir, dataSourceName, err)
	}
...
func (e *Engine) Prepare(query string) (*Stmt, error) {
	...
	if err != nil {
		return nil, core.Error(core.GenericError, "cannot prepare statement", err)
	}
	return stmt, nil
}
