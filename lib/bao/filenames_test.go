...
func TestEncodeDecodeHead(t *testing.T) {
	...
	file := File{
		...
		Attrs:     attrs,
		LocalCopy: "test.txt",
		StoreDir:  "storage/dir",
		StoreName: "storage/test.txt",
		AuthorId:  alice.PublicIDMust(),
		KeyId:     1, // Use a non-zero key ID for encryption
	}
	head, err := encodeHead(file, alice, func(keyId uint64) (security.AESKey, error) {
		return aesKey, nil
	}, func(keyId uint64) (Group, error) {
		return Users, nil
	})
	...
	keys[1] = aesKey
	file, err = decodeHead(head, alice, func(keyId uint64) (security.AESKey, error) {
		return aesKey, nil
	})
