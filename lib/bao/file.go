...
type File struct {
	...
	Group         Group             `json:"group"`           // Group to which the file belongs
	...
	StoreDir      string            `json:"storageDir"`      // Directory in the storage where the file is located
	StoreName     string            `json:"storageName"`     // Name of the file in the storage
	AuthorId      security.PublicID `json:"authorId"`        // Author ID of the file
}

// queryFileById retrieves a file by its ID from the database.
func (s *Bao) queryFileById(fileId FileId) (file File, ok bool, err error) {
	...
	var dir, name string
	err = s.DB.QueryRow("GET_FILE_BY_ID", sqlx.Args{"store": s.Id, "id": fileId},
		&file.Id, &file.StoreDir, &file.StoreName, &dir, &name, &file.Group, &file.LocalCopy,
...
func (s *Bao) queryFileByName(name string) (file File, ok bool, err error) {
	...
	if colonIdx == 0 {
		...
		return s.queryFileById(FileId(id))
	} else if colonIdx >= 1 {
	...
	var modTimeUnix int64
	err = s.DB.QueryRow("GET_FILE_BY_NAME", sqlx.Args{"store": s.Id, "dir": dir, "name": name,
		"version": version},
		&file.Id, &dir, &file.Name, &file.Group, &file.StoreDir, &file.StoreName, &file.LocalCopy,
		&modTimeUnix, &file.Size, &file.AllocatedSize, &file.Flags, &file.AuthorId, &file.KeyId, &file.Attrs)
	if err != nil {
		if err == sqlx.ErrNoRows {
			core.End("file not found")
...
func (s *Bao) updateFileLocalName(fileId FileId, localCopy string) error {
	core.Start("updating local name for file %d to %s", fileId, localCopy)
	// Update the local name of the file in the database
	_, err := s.DB.Exec("UPDATE_FILE_LOCAL_NAME", sqlx.Args{"store": s.Id, "id": fileId, "localCopy": localCopy})
	if err != nil {
...
func (s *Bao) queryFileIdsByFlags(flags Flags) ([]FileId, error) {
	core.Start("querying file IDs by flags %d", flags)
	// Query the database for file IDs with the specified flags
	//	rows, err := s.DB.Query("GET_FILE_IDS_BY_FLAGS", sqlx.Args{"store": s.Id, "flags": flags})
	rows, err := s.DB.Query("GET_FILE_IDS_BY_FLAGS", sqlx.Args{"store": s.Id, "flagsMask": flags})
	if err != nil {
