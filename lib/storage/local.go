...
func OpenLocal(id string, connectionUrl string) (Store, error) {
	...
	if err != nil {
		core.Errorw("invalid URL: %v", err)
		return nil, core.Errorw("invalid URL", err)
	}

	if u.Scheme != "file" {
		return nil, core.Errorw("invalid scheme: %s", u.Scheme)
	}

	if u.Host != "" {
		return nil, core.Errorw("invalid host: %s", u.Host)
	}

	core.End("Opened local storage with path: %s", u.Path)
...
func (l *Local) Close() error {
	core.Info("closing local storage")
	return nil
}
