...
func (v *Vault) readFile(file File, progress chan int64) error {
	...
	if err != nil {
		return core.Error(core.FileError, "cannot read file %s", file.Name, err)
	}
	...
	if file.Flags&PendingRead != 0 {
		...
		if err != nil {
			return core.Error(core.FileError, "cannot clear PendingRead flag for file %s", file.Name, err)
		}
	}
...
func (v *Vault) UpdateFileAllocatedSize(id FileId, allocatedSize int64) error {
	...
	if err != nil {
		return core.Error(core.DbError, "cannot update file allocated size for id %d", id, err)
	}
...
func (v *Vault) UpdateFileFlags(id FileId, flags Flags) error {
	...
	if err != nil {
		return core.Error(core.DbError, "cannot update file flags for id %d", id, err)
	}
...
func (v *Vault) Read(name string, dest string, options IOOption, progress chan int64) (File, error) {
	...
	if err != nil {
		return File{}, core.Error(core.DbError, "cannot update local name for file %s", name, err)
	}
	...
	default: // Default case, read the file synchronously
		...
		if err != nil {
			return File{}, core.Error(core.GenericError, "cannot read file %s", name, err)
		}
